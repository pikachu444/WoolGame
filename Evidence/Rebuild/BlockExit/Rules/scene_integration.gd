...
var workspace="C:/SourceCodes/WoolGame/BuildWork/BlockExitRules"
...
func test_boundaries():
 ...
 for _i in range(120):a.advance(1.0/60.0)
 for _i in range(20):b.advance(0.1)
 check(a.hearts==b.hearts and a.lost==b.lost and a.cat_phase==b.cat_phase and abs(a.time-b.time)<0.5,"delta time consistent encounter outcome",[a.snapshot(),b.snapshot()])
...
func test_real_failure():
 for level in range(10):
  ...
  var damage_time=s.time;check(s.continue_run(),"natural fire loss accepts continue "+str(level));var result=solve(s,damage_time+420.0)
  if not s.won:
   s=fresh(level,{"max_hearts":3 if level>=6 else 2 if level>=3 else 1,"shield":level>=2,"freeze":level>=7});s.advance(300);s.continue_run();result=solve(s,s.time+420.0,0.4)
  check(s.won,"natural failure recovery tool free "+str(level),result)
...
func run():
 var scene=load("res://scenes/game.tscn").instantiate();root.add_child(scene);await process_frame
 # Audio playback is outside this headless progression audit.
 scene.muted=true
 check(scene.screen=="home","integration opens home")
 scene.start_stage(1);check(scene.screen=="home","integration locked stage refused")
 var stages=[]
 for mode in ["challenge","free"]:
  scene.switch_mode(mode)
  for level in range(10):
   scene.start_stage(level);check(scene.state.level_index==level and scene.screen=="play","integration stage starts "+mode+str(level))
   var result=solve(scene.state);check(scene.state.won,"integration complete "+mode+str(level),result)
   check(not scene.receipt.is_empty() and scene.profile.current().pending.run_id==scene.run_id,"integration victory immediately settled "+mode+str(level))
   var disk=Profile.new();disk.open(scene.profile.path)
   check(disk.current().settled.has(scene.run_id) and disk.current().completed.has(level),"integration victory persisted before result navigation "+mode+str(level))
   check(disk.current().coins==300+50*(level+1),"integration cumulative reward "+mode+str(level),disk.current().coins)
   var expected={"max_hearts":3 if level>=5 else 2 if level>=2 else 1,"shield":level>=1,"freeze":level>=6}
   check(disk.stats()==expected,"integration growth reload "+mode+str(level),disk.stats())
   stages.append({"mode":mode,"level":level,"time":scene.state.time,"receipt":scene.receipt.duplicate(true)})
   scene.acknowledge_result();check(scene.screen=="home" and scene.profile.current().pending.is_empty(),"integration result acknowledge home")
 check(scene.profile.current("challenge").coins==800 and scene.profile.current("free").coins==800,"integration mode wallets independent")
 scene.switch_mode("challenge");scene.start_stage(0);solve(scene.state);check(scene.receipt.coins==20 and not scene.receipt.first and scene.profile.current().cards.size()==10,"integration replay reward after campaign")
 var file=FileAccess.open(out+"/scene-integration.json",FileAccess.WRITE);file.store_string(JSON.stringify({"checks":checks,"failures":failures,"stages":stages,"audio_muted_in_harness":true},"  "));file.close()
 print("INDEPENDENT_SCENE checks=",checks," failures=",failures.size())
