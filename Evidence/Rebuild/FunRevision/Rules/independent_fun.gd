...
func play(level,interval,first_id,policy):
 ...
 return {"level":level+1,"interval":interval,"first":first_id,"policy":policy,"won":s.won,"lost":s.lost,"time":s.time,"hearts":s.hearts,"collected":s.collected,"total":s.total,"loss_reason":s.loss_reason,"longest_no_capture":longest_idle,"actions":actions,"end":{} if s.won else s.snapshot()}
...
func test_stall_timer():
 var actions=[19,26,0,13,12,25,18,14,15];var s=natural(9);var index=0;var events=[];var prior_count=0
 var capture_event=func(_u,_b):events.append(s.time)
 s.captured.connect(capture_event)
 while not s.lost and s.time<40:
  if index<actions.size() and s.time+0.001>=2.4*(index+1):s.select(actions[index]);index+=1
  s.advance(0.1)
  if s.collected>prior_count:
   check(s.stalled_since<0 or s.stalled_since>=events[-1],"actual capture resets stall timer",[s.time,s.stalled_since,events[-1]])
   prior_count=s.collected
 check(not s.lost and s.collected>25,"previous progress loss repaired",{"time":s.time,"collected":s.collected,"reason":s.loss_reason})
 s.captured.disconnect(capture_event)
 s=natural(9)
 for i in range(4):s.slots[i]=i;s.blocks[i].slot=i;s.blocks[i].phase="working";s.blocks[i].next=0
 for dragon in s.dragons:dragon.head=-5000
 s.freeze_until=100
 s.advance(7.9);check(not s.lost and s.collected==0,"genuine no-capture stall grace interval")
 s.advance(0.3);check(s.lost and s.collected==0 and s.hearts==3,"genuine no-capture stall still loses after eight seconds",s.snapshot())
func run():
 audit_definitions();test_profile();test_continue_preservation();test_stall_timer()
 for level in [5,7,9]:
