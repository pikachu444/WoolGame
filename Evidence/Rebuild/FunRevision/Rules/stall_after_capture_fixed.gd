...
func _initialize():
 ...
 var f=FileAccess.open("C:/SourceCodes/WoolGame/Evidence/Rebuild/FunRevision/Rules/stall-after-capture-fixed.json",FileAccess.WRITE);f.store_string(JSON.stringify(result,"  "));f.close();print(JSON.stringify(result));quit()
