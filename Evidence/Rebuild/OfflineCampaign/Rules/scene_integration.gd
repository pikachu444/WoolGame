...
func run():
 ...
 var file=FileAccess.open(out+"/scene-integration.json",FileAccess.WRITE);file.store_string(JSON.stringify({"checks":checks,"failures":failures,"stages":stages},"  "));file.close()
