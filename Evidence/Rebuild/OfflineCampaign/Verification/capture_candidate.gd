...
func run() -> void:
	...
	game.start_stage(1,true);next=0.0
	while s.time<200 and not s.won and not s.lost:
		if s.time>=next:
			var id=s.request_hint()
			if id>=0 and s.available(s.blocks[id].color):s.select(id)
			next=s.time+0.8
		s.advance(0.05)
	s.advance(2.1);game._process(0);await shot("15-first-shield-unlock")
	game.acknowledge_result();game.navigate("growth");await shot("16-growth-earned")
