...
func _initialize():
 var hashes={}
 for name in ["state","reference_stage","campaign"]:hashes[name]=FileAccess.get_sha256("res://scripts/"+name+".gd")
 test_optimization_and_preservation();test_reading_and_approach()
 runs.append(simulate(true))
 for interval in [0.8,1.2,2.4,3.2]:
  for policy in [0,1]:runs.append(simulate(false,interval,policy,true))
 runs.append(simulate(false,1.2,1,false))
 var file=FileAccess.open("C:/SourceCodes/WoolGame/Evidence/Rebuild/Representative/Rules/final-replay.json",FileAccess.WRITE);file.store_string(JSON.stringify({"checks":checks,"failures":failures,"runs":runs,"source_sha256":hashes,"source_input":"level2_replay_observed.json, not developer observed_actions or choose","definitions_baseline":"8718affc22bea206ddb5af8acaf44a3aee02a3d0"},"  "));file.close();print("INDEPENDENT_FINAL_REPLAY checks=",checks," failures=",failures.size());quit(0 if failures.is_empty() else 1)
