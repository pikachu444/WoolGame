...
func test_windup():
 ...
 for u in s.units:u.erase("power")
 s.cat_anchor_index=s.definition.anchors.size()-1;s.cat_position=s.definition.anchors[-1];var d=s.dragons[0];var target=d.curve.get_closest_offset(s.cat_position);d.head=target-90;d.phase="chasing";d.until=0
...
func test_stage_markers():
 ...
 records.append({"case":"actual stage marker wiring","markers":declared,"note":"Observed block capture ordinal and full route replay deferred until candidate freeze"})
func _initialize():
 var hashes_start={}
 for path in ["state","campaign","reference_stage"]:hashes_start[path]=FileAccess.get_sha256("res://scripts/"+path+".gd")
 test_trigger();test_repel();test_slow();test_continue_and_serialization();test_windup();test_other_stages();test_recovery();test_stage_markers()
 ...
 var report={"source_sha256_at_start":hashes_start,"source_sha256_at_end":loaded,"scope":"effect unit snapshot; controlled existing-unit marker fixture; final stage replay deferred","checks":checks,"failures":failures,"records":records,"stage_has_yarn_powers":Campaign.definition(1).has("yarn_powers"),"stage_powers":Campaign.definition(1).get("yarn_powers",[]),"save_limit":"Game has no mid-run State restore API; continue and snapshot serialization only were tested."}
 var f=FileAccess.open("C:/SourceCodes/WoolGame/Evidence/Rebuild/Representative/Rules/power_units_snapshot.json",FileAccess.WRITE);f.store_string(JSON.stringify(report,"  "));f.close();print(JSON.stringify(report));quit(0 if failures.is_empty() else 1)
