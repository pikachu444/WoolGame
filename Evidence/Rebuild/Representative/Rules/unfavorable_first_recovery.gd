...
func simulate(observed,interval=1.2,policy=0,guard=true,forced_first=-1):
 ...
 while not s.won and not s.lost and s.time<240:
  if observed and next_index<source.events.size() and s.time+0.00001>=float(source.events[next_index].relative_active_seconds):
   ...
   if s.slots.has(-1):
    var id=forced_first if actions.is_empty() and forced_first>=0 else pick(s,policy,guard)
    if id>=0:
...
func _initialize():
 var probe=State.new();probe.reset(0,1);probe.advance(3.2);var initially_visible=visible_count(probe)
 var trial=simulate(false,3.2,0,true,1)
 var file=FileAccess.open("C:/SourceCodes/WoolGame/Evidence/Rebuild/Representative/Rules/unfavorable-first-recovery.json",FileAccess.WRITE);file.store_string(JSON.stringify({"first_block":"b02","initial_same_color_visible":initially_visible.get(0,0),"normal_selection":true,"run":trial,"checks":checks,"failures":failures,"state_sha256":FileAccess.get_sha256("res://scripts/state.gd"),"reference_sha256":FileAccess.get_sha256("res://scripts/reference_stage.gd")},"  "));file.close();quit(0 if failures.is_empty() else 1)
