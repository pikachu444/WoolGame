...
- tests/verify_campaign.gd / verify_ui.gd: 현재 캠페인의 규칙 및 씬 전환 회귀 검사.
...
빌드는 아트 리소스를 만들고 두 회귀 검사를 실행한 뒤 Windows ZIP과 ARM64 APK를 Builds/Rebuild에 생성한다. 같은 PCK가 두 패키지에 들어간다. APK 실행 진입점, 서명, 16KB 정렬, 내장 PCK 일치를 확인한다. 배포 APK는 개인 설치용 디버그 서명이며 Android 8 이상 ARM64용이다.
...
검증 기록은 [OfflineCampaign](../Evidence/Rebuild/OfflineCampaign)에 모은다. ReferenceMatch/Final은 이전 빌드의 기록이다. 현재 실제 UI·기기 검증 범위와 후보 해시는 OfflineCampaign/ACCEPTANCE.md에서 확인한다.
