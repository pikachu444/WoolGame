        ...
        Pass
        {
            ...
            half4 frag(V i):SV_Target
            {
                ...
                if(_HasTextures>.5) {
                    ...
                    ao=lerp(.52,1,saturate((SAMPLE_TEXTURE2D(_OcclusionMap,sampler_OcclusionMap,tiled).r-_AOFloor)/max(.01,1-_AOFloor)));
                }
                ...
                float broad=saturate(dot(normalize(i.normal),l.direction)); float3 light=float3(.18,.20,.24)+l.color*(broad*.80+ndl*.25+.05)*lerp(.20,1,l.shadowAttenuation);
