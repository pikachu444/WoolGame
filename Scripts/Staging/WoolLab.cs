...
{
    public sealed class WoolLab : MonoBehaviour
    {
        ...
        public void BuildPresentation()
        {
            ...
            var lightGo=new GameObject("Soft window light");lightGo.transform.SetParent(stage);lightGo.transform.rotation=Quaternion.Euler(53,-32,0);var l=lightGo.AddComponent<Light>();l.type=LightType.Directional;l.intensity=1.0f;l.color=new Color(1,.98f,.96f);l.shadows=LightShadows.Soft;l.shadowStrength=.72f;l.shadowBias=.015f;l.shadowNormalBias=.03f;
            yarn=new Material[4];for(int i=0;i<4;i++)yarn[i]=MakeYarn(settings.yarnColors[i]);
            cream=MakeYarn(WoolLabSettings.Hex("EEE4D1"));wood=Lit(WoolLabSettings.Hex("CDAF91"),.35f);ink=Lit(settings.ink,.18f);ground=Linen(WoolLabSettings.Hex("DEE7EE"),48);
        ...
        {
            var m=Own(new Material(settings.surfaceShader?settings.surfaceShader:Shader.Find("Universal Render Pipeline/Lit")));m.SetColor("_BaseColor",color.linear);m.SetFloat("_Smoothness",smooth);return m;
        }
        ...
        Material MakeYarn(Color color)
        {
            ...
            m.SetFloat("_AOFloor",settings.physicalKnit?0:.72f);if(settings.physicalKnit)m.SetFloat("_BumpScale",.60f);
        ...
        void BuildStage()
        {
            Vector3[] curve={World(.10f,.727f),World(.10f,.817f),World(.22f,.862f),World(.58f,.867f),World(.83f,.835f),World(.88f,.764f),World(.71f,.709f),World(.47f,.732f)};
            path=new WoolPath(curve);
            var pathMat=Linen(WoolLabSettings.Hex("D8CCD9"),18);var ribbon=OwnTube(new WoolTube("Felt path piping",stage,pathMat,120,8));
            for(int i=0;i<ribbon.Points.Length;i++){var p=path.At((float)i/(ribbon.Points.Length-1)*path.Length);p.y=0;ribbon.Points[i]=p;}ribbon.Update(.40f);ribbon.GameObject.transform.localScale=new Vector3(1,.10f,1);ribbon.GameObject.transform.position=Vector3.down*.28f;
            ...
            dragon=MakeActor(settings.dragonModel,true);dragonScale=Vector3.one*settings.bodyWidth*1.25f;dragon.localScale=dragonScale;
            cat=MakeActor(settings.catModel,false);cat.localScale=Vector3.one*1.40f;catHome=World(.48f,.792f);catHome.y=.10f;cat.position=catHome;cat.rotation=Quaternion.Euler(0,180,0);wavingPaw=FindPart(cat,"wavepaw");
            eyeLeft=FindPart(cat,"eyel");eyeRight=FindPart(cat,"eyer");if(eyeLeft)eyeLeftScale=eyeLeft.localScale;if(eyeRight)eyeRightScale=eyeRight.localScale;if(wavingPaw)pawRest=wavingPaw.localRotation;
            var catSeat=WoolGeometry.MeshObject("Cat's quilted cushion",stage,box,MakeYarn(WoolLabSettings.Hex("D8C6DA")),catHome-new Vector3(0,.13f,0),new Vector3(1.65f,.25f,1.30f));
            var boardMat=Linen(WoolLabSettings.Hex("F3EDE4"),10);var boardCenter=World(.5f,.348f);boardCenter.y=-.26f;
            Vector3 traySize=new Vector3(Vector3.Distance(World(.16f,.35f),World(.84f,.35f)),.32f,Vector3.Distance(World(.5f,.165f),World(.5f,.535f)));
            WoolGeometry.MeshObject("Rounded sewing tray rim",stage,Own(WoolGeometry.Tray(traySize,.65f)),Lit(WoolLabSettings.Hex("C8B6AB"),.1f),boardCenter,Vector3.one);
            var innerSize=new Vector3(traySize.x-.26f,.15f,traySize.z-.26f);
            WoolGeometry.MeshObject("Inset linen playing surface",stage,Own(WoolGeometry.Tray(innerSize,.55f)),boardMat,boardCenter+Vector3.up*.12f,Vector3.one);
            var benchPosition=World(.5f,.598f);benchPosition.y=-.26f;float benchWidth=Vector3.Distance(World(.10f,.6f),World(.90f,.6f));
            WoolGeometry.MeshObject("Maple collecting bench",stage,Own(WoolGeometry.Tray(new Vector3(benchWidth,.20f,1.5f),.35f)),wood,benchPosition,Vector3.one);
            for(int s=0;s<4;s++){var pad=World(.20f+s*.20f,.598f);pad.y=-.11f;WoolGeometry.MeshObject("Inset spool seat "+s,stage,Own(WoolGeometry.Tray(new Vector3(benchWidth*.21f,.06f,1.15f),.25f)),Lit(WoolLabSettings.Hex("C8C4CB"),.05f),pad,Vector3.one);}
            int[][] footprints={new[]{2,4,1,1},new[]{1,2,1,1},new[]{3,2,1,2},new[]{2,0,3,1}};
            for(int i=0;i<4;i++)
            {
                int[] f=footprints[i];Vector3 p=Grid(f[0]+(f[2]-1)*.5f,f[1]+(f[3]-1)*.5f);p.y=.16f;blockStart[i]=p;
                float w=Vector3.Distance(Grid(0,0),Grid(1,0)),h=Vector3.Distance(Grid(0,0),Grid(0,1));
                Vector3 size=new Vector3(w*f[2]-.12f,.39f,h*f[3]-.12f);
                blocks[i]=WoolGeometry.MeshObject("Direction block "+i,stage,Own(WoolGeometry.RoundedBox(size,.16f,8,settings.bodyWidth)),yarn[i],p,Vector3.one).transform;
                var col=blocks[i].gameObject.AddComponent<BoxCollider>();col.size=size;var tag=blocks[i].gameObject.AddComponent<WoolBlockTag>();tag.id=i;
                spools[i]=new GameObject("Working spool "+i).transform;spools[i].SetParent(stage,false);
                WoolGeometry.MeshObject("Spindle",spools[i],box,wood,Vector3.zero,new Vector3(1.12f,.23f,.23f));
                for(int end=-1;end<=1;end+=2){var disc=GameObject.CreatePrimitive(PrimitiveType.Cylinder);disc.name="Maple flange";disc.transform.SetParent(spools[i],false);disc.transform.localPosition=new Vector3(end*.59f,0,0);disc.transform.localRotation=Quaternion.Euler(0,0,90);disc.transform.localScale=new Vector3(.88f,.065f,.88f);disc.GetComponent<Renderer>().sharedMaterial=wood;Destroy(disc.GetComponent<Collider>());}
                coils[i]=OwnTube(new WoolTube("Wound yarn "+i,stage,yarn[i],112,6));threads[i]=OwnTube(new WoolTube("Traveling yarn "+i,stage,yarn[i],42,8));
                woundBodies[i]=OwnTube(new WoolTube("Growing wound core "+i,stage,yarn[i],3,16));sourceLoops[i]=OwnTube(new WoolTube("Last releasing stitch "+i,stage,MakeYarn(Color.Lerp(settings.yarnColors[i],Color.white,.35f)),15,6));
            }
        }
            ...
            if(prefab){var actor=Instantiate(prefab,stage);actor.name=isDragon?"Som Cloud Dragon":"Cream Cat";
                ...
                foreach(var r in actor.GetComponentsInChildren<Renderer>()){var mats=r.sharedMaterials;for(int j=0;j<mats.Length;j++){
                    ...
                    if(!palette.TryGetValue(name,out var material)){
                        ...
                        else if(name.Contains("eye")||name.Contains("pupil")||name.Contains("mouth"))material=ink;
                        ...
                        else if(name.Contains("scarf"))material=MakeYarn(WoolLabSettings.Hex("B49CC8"));
                        else if(isDragon&&name.Contains("wool"))material=MakeYarn(WoolLabSettings.Hex("A4CCD5"));
                        else material=cream;
        ...
        Vector3 Grid(float x,float y)=>World(.16f+(x+.5f)*.123f,.185f+(y+.5f)*.063f);
        ...
        void ResetVisualState()
        {
            ...
            for(int i=0;i<4;i++){feedbackAt[i]=-10;lastAmount[i]=0;returnPaths[i]=null;blockLabels[i].transform.localScale=Vector3.one;blocks[i].position=blockStart[i];blocks[i].localScale=Vector3.one;blocks[i].gameObject.SetActive(true);spools[i].gameObject.SetActive(false);threads[i].GameObject.SetActive(false);coils[i].GameObject.SetActive(false);woundBodies[i].GameObject.SetActive(false);sourceLoops[i].GameObject.SetActive(false);blockLabels[i].gameObject.SetActive(true);blockLabels[i].text="\n<size=21><b>"+Session.Blocks[i].Capacity+"</b></size>";slotLabels[i].text="·";}
            pausePanel.SetActive(false);successPanel.SetActive(false);cat.position=catHome;cat.localScale=Vector3.one*1.40f;dragon.localScale=dragonScale;dragon.gameObject.SetActive(true);UpdatePresentation(0);
        }
        ...
        void UpdatePresentation(float dt)
        {
            ...
            if(Session.Won&&time>=latestCapture+.50f)
            {
                ...
                float p=Mathf.Clamp01((time-successAt)/1.2f);cat.position=catHome+(Vector3.up*.65f+Vector3.forward*.18f)*Mathf.Sin(p*Mathf.PI);cat.localScale=Vector3.one*1.40f+new Vector3(-.08f,.14f,-.08f)*Mathf.Sin(p*Mathf.PI*4)*(1-p);
        ...
        void UpdateBlockAndSpool(int b,float time)
        {
            ...
            float fraction=visualAmount/block.Capacity;float turns=Mathf.Max(.02f,14*fraction);Vector3 end=slotPosition[block.Slot];
            float coreRadius=.12f+.18f*Mathf.Sqrt(fraction);for(int j=0;j<3;j++)woundBodies[b].Points[j]=slotPosition[block.Slot]+Vector3.right*Mathf.Lerp(-.47f,.47f,j*.5f);if(visible&&visualAmount>.01f)woundBodies[b].Update(coreRadius*(1-completion));
            for(int j=0;j<coils[b].Points.Length;j++){float q=(float)j/(coils[b].Points.Length-1);float a=q*turns*Mathf.PI*2;float x=Mathf.Lerp(-.47f,.47f,q);end=slotPosition[block.Slot]+new Vector3(x,Mathf.Cos(a)*(coreRadius+.03f),Mathf.Sin(a)*(coreRadius+.03f));coils[b].Points[j]=end;}if(visible&&visualAmount>.01f)coils[b].Update(.052f*(1-completion));
            threads[b].GameObject.SetActive(visible&&activeUnit>=0);
            sourceLoops[b].GameObject.SetActive(visible&&activeUnit>=0);
            if(visible&&activeUnit>=0){float peel=Mathf.Clamp01((time-capturedAt[activeUnit])/.45f);float edgeZ=.35f-.70f*peel;
                Vector3 source=units[activeUnit].TransformPoint(new Vector3(0,.17f+.12f*Mathf.Exp(-Mathf.Pow((edgeZ-.17f)/.14f,2)),edgeZ));
                ...
                Vector3 bend=World(side,Mathf.Clamp(sy,.69f,.91f)),approach=World(side,.625f);bend.y=.30f;approach.y=.32f;
                threads[b].Cubic(source,bend,approach,end,.033f);
            }
            lastAmount[b]=visualAmount;
            if(visible){slotLabels[block.Slot].text=counterText[Mathf.Clamp(block.Capacity-Mathf.FloorToInt(visualAmount),0,10)];slotLabels[block.Slot].color=settings.ink.linear;}else if(finished&&SlotHasNoActiveBlock(block.Slot))slotLabels[block.Slot].text="·";
        }
        ...
        void BuildUI()
        {
            ...
            var go=new GameObject("WoolLab Canvas");go.transform.SetParent(stage,false);canvas=go.AddComponent<Canvas>();canvas.renderMode=RenderMode.ScreenSpaceOverlay;go.AddComponent<GraphicRaycaster>();var scaler=go.AddComponent<CanvasScaler>();scaler.uiScaleMode=CanvasScaler.ScaleMode.ScaleWithScreenSize;scaler.referenceResolution=new Vector2(720,1560);scaler.matchWidthOrHeight=.5f;
            safeUI=new GameObject("Safe Area",typeof(RectTransform)).GetComponent<RectTransform>();safeUI.SetParent(go.transform,false);safeUI.anchorMin=Vector2.zero;safeUI.anchorMax=Vector2.one;safeUI.offsetMin=safeUI.offsetMax=Vector2.zero;
            Label("솜길 구조대",.09f,.958f,32,settings.ink,TextAnchor.MiddleLeft,370,65).fontStyle=FontStyle.Bold;
            Panel(safeUI,.83f,.955f,132,49,WoolLabSettings.Hex("FFF8E9"));
            progress=Label("0 / 24",.83f,.955f,21,settings.ink,TextAnchor.MiddleCenter,120,55);progress.fontStyle=FontStyle.Bold;
            Label("실을 풀어 고양이를 구해 주세요",.09f,.919f,15,settings.ink,TextAnchor.MiddleLeft,450,34);
            Label("실 모으는 자리",.5f,.66f,19,settings.ink,TextAnchor.MiddleCenter,400,38).fontStyle=FontStyle.Bold;
            for(int i=0;i<4;i++){Panel(safeUI,.20f+i*.20f,.60f,112,81,new Color(.70f,.63f,.57f,.12f));slotLabels[i]=Label("·",.20f+i*.2f,.563f,22,settings.ink,TextAnchor.MiddleCenter,80,38);blockLabels[i]=Label("",.5f,.3f,37,Color.white,TextAnchor.MiddleCenter,95,95);blockLabels[i].gameObject.AddComponent<Shadow>().effectDistance=new Vector2(0,-2);
                var arrow=new GameObject("Original arrow",typeof(RectTransform),typeof(Image));var ar=arrow.GetComponent<RectTransform>();ar.SetParent(blockLabels[i].transform,false);ar.anchorMin=ar.anchorMax=new Vector2(.5f,.58f);ar.sizeDelta=new Vector2(37,37);ar.anchoredPosition=Vector2.zero;ar.localRotation=Quaternion.Euler(0,0,-90*i);var ai=arrow.GetComponent<Image>();ai.sprite=ArrowSprite();ai.raycastTarget=false;
            }

            status=Label("화살표를 따라, 한 올씩 풀어 주세요",.5f,.128f,18,settings.ink,TextAnchor.MiddleCenter,650,45);
            pauseLabel=Button("잠깐",.23f,.074f,124,54,TogglePause);Button("다시 보기",.50f,.074f,166,54,Replay);speedLabel=Button("0.5×",.77f,.074f,124,54,()=>{playbackSpeed=playbackSpeed==1?.5f:1;speedLabel.text=playbackSpeed==1?"0.5×":"1×";});
            Label("OUR PROTOTYPE · 광고 없이, 포근하게",.5f,.028f,12,new Color(.57f,.51f,.59f),TextAnchor.MiddleCenter,620,25);
            pausePanel=Panel(safeUI,.5f,.40f,570,310,new Color(.98f,.95f,.92f,.98f)).gameObject;
            var title=Label("잠시 쉬어 가요",.5f,.44f,29,settings.ink,TextAnchor.MiddleCenter,510,60);title.transform.SetParent(pausePanel.transform,true);
            var resume=Button("계속하기",.5f,.385f,210,55,TogglePause);resume.transform.parent.SetParent(pausePanel.transform,true);
            var step=Button("한 걸음",.80f,.385f,100,45,Step);step.transform.parent.SetParent(pausePanel.transform,true);
            soundLabel=Button("소리 꺼짐",.34f,.329f,150,45,()=>{muted=!muted;soundLabel.text=muted?"소리 꺼짐":"소리 켜짐";});soundLabel.transform.parent.SetParent(pausePanel.transform,true);
            hapticLabel=Button("진동 꺼짐",.66f,.329f,150,45,()=>{haptic=!haptic;hapticLabel.text=haptic?"진동 켜짐":"진동 꺼짐";});hapticLabel.transform.parent.SetParent(pausePanel.transform,true);
            pausePanel.SetActive(false);
            successPanel=Panel(safeUI,.5f,.36f,535,230,new Color(.98f,.96f,.91f,.98f)).gameObject;
            var successTitle=Label("포근하게, 구조 성공!",.5f,.383f,30,settings.ink,TextAnchor.MiddleCenter,520,65);successTitle.transform.SetParent(successPanel.transform,true);
            var successSub=Label("24개의 실이 작은 안심이 되었어요",.5f,.342f,17,settings.ink,TextAnchor.MiddleCenter,520,45);successSub.transform.SetParent(successPanel.transform,true);successPanel.SetActive(false);
            successTitle.fontStyle=FontStyle.Bold;
            for(int h=0;h<rescueHearts.Length;h++){rescueHearts[h]=Label("♥",.48f,.795f,22,settings.yarnColors[0],TextAnchor.MiddleCenter,35,35);rescueHearts[h].gameObject.SetActive(false);}
            if(UnityEngine.EventSystems.EventSystem.current==null){var es=new GameObject("Input Event System");es.AddComponent<UnityEngine.EventSystems.EventSystem>();es.AddComponent<UnityEngine.InputSystem.UI.InputSystemUIInputModule>();}
        }
        ...
        {
            var r=Panel(safeUI,x,y,w,h,WoolLabSettings.Hex("E5DCE8"));var b=r.gameObject.AddComponent<Button>();b.onClick.AddListener(action);var text=Label(title,x,y,20,settings.ink,TextAnchor.MiddleCenter,w,h);text.fontStyle=FontStyle.Bold;text.transform.SetParent(r,true);return text;
        }
    }
